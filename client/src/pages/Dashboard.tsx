...
const Dashboard = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto mt-8 px-4 pb-12 font-comfortaa flex flex-col items-center">
      ...
      {/* Filtros */}
      <div className="w-full max-w-[900px] flex flex-col gap-6 mb-12 surface-card p-10 rounded-2xl shadow-xl border-1 border-primary-500 bg-opacity-50">
        ...
        {selectedStage === 'REGULAR' && (
            <div className="flex items-center gap-4 justify-center md:justify-start border-t border-gray-700 pt-8">
                ...
                <Dropdown 
                    ...
                    className="w-full md:w-16rem p-inputtext-sm font-bold shadow-4"
                />
            </div>
        )}
      </div>
      ...
      {filteredMatchups.length === 0 ? (
          <div className="w-full max-w-[900px] text-center p-20 surface-card rounded-2xl border-1 border-dashed border-gray-600">
          ...
          <div className="w-fit mx-auto pb-12 px-2 md:px-10">
              <DataTable 
                ...
                className="shadow-8 rounded-2xl overflow-hidden border-1 border-gray-800"
              >
                {/* Away Team Section */}
                <Column header="" body={(row) => (
                    <div className="py-6 px-4">
                        <img src={row.awayTeam.logoUrl} className="w-12 h-12 object-contain" alt="" />
                    </div>
                )} style={{width: '6rem'}} align="right" headerClassName="justify-content-center"></Column>
                
                <Column header={t('admin.form.away')} body={(row) => (
                    <div className="py-6 px-2">
                        <span className={`text-xs md:text-sm  tracking-widest font-black ${row.isFinished && row.winnerId === row.awayTeamId ? 'text-primary' : 'text-gray-400'}`}>{row.awayTeam.name}</span>
                    </div>
                )} style={{width: '12rem'}} headerClassName="text-center"></Column>
                
                {/* Pick Away */}
                <Column header="" body={(row) => (
                    <div className="py-6 px-4">
                        {row.isFinished ? <span className="text-3xl font-black">{row.awayScore}</span> :
                        <Checkbox 
                ...
                <Column header="" body={(row) => (
                    <div className="py-6 px-4">
                        {row.isFinished ? <Tag value="FINAL" severity="secondary" className="text-[10px] font-black px-3 py-1" /> : <span className="text-gray-600 font-black text-xl italic opacity-50">@</span>}
                    </div>
                )} align="center" style={{width: '6rem'}} headerClassName="justify-content-center"></Column>
                ...
                <Column header="" body={(row) => (
                    <div className="py-6 px-4">
                        {row.isFinished ? <span className="text-3xl font-black">{row.homeScore}</span> :
                        <Checkbox 
                ...
                <Column header={t('admin.form.home')} body={(row) => (
                    <div className="py-6 px-2">
                        <span className={`text-xs md:text-sm  tracking-widest font-black ${row.isFinished && row.winnerId === row.homeTeamId ? 'text-primary' : 'text-gray-400'}`}>{row.homeTeam.name}</span>
                    </div>
                )} style={{width: '12rem'}} align="left" headerClassName="justify-content-center text-center"></Column>
                
                <Column header="" body={(row) => (
                    <div className="py-6 px-4">
                        <img src={row.homeTeam.logoUrl} className="w-12 h-12 object-contain" alt="" />
                    </div>
                )} style={{width: '6rem'}} align="left" headerClassName="justify-content-center"></Column>
                    ...
                    return (
                        <div className="py-6 px-6">
                            <Tag 
                                ...
                                className="text-[9px] font-black italic px-4 py-2 shadow-4 tracking-widest"
                            />
                        </div>
                    )
                }} align="center" style={{width: '14rem'}} headerClassName="justify-content-center"></Column>
              </DataTable>
          </div>
      )}
    </div>
